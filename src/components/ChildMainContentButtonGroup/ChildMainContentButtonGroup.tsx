...
import { useDeleteRowMutation, useFetchEntityDataQuery } from "../../Redux/api";
import { useDispatch, useSelector } from "react-redux";
import { dataList, disableButton } from "../../Redux/appStateSlice";
import { ChildMainContentButtonGroupProps } from ".";
import { AppState } from "../../Redux/redux.types";

import createRowLocal from "../../MainContent/MainContent.service";

import "./ChildMainContentButtonGroup.style.scss";

const ChildMainContentButtonGroup: React.FC<
  ChildMainContentButtonGroupProps
> = ({ itemId }) => {
  const [deleteRow, { isSuccess: isSuccessDelete, error }] =
    useDeleteRowMutation();

  const { refetch } = useFetchEntityDataQuery(null);

  const isButtonDisabled = useSelector(
    (state: { appState: AppState }) => state.appState.disableButton
  );
  ...
  const handleButtonClickCreate = () => {
    createRowLocal(itemId, allData, dispatch, dataList, disableButton);
...
export default ChildMainContentButtonGroup;
